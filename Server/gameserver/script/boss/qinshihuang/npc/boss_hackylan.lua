...
-- 秦始皇boss
local tbQinshihuangBoss	= Npc:GetClass("boss_hackylan");
...
function tbQinshihuangBoss:DeathLoseItem(tbLoseItem)
	...
	local szMsg = "<color=green>Hắc Kỳ Lân rơi vật phẩm:<color>\n";
	local tbList = {};
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5, "boss_hackylan");
	pPlayer.AddExp(30000000)
		...
		pPlayer.AddItem(22,1,81,1); --hòa thị ngọc
		pPlayer.AddItem(22,1,81,1); --hòa thị ngọc
		pPlayer.Msg("<color=green>Hành trang trống >1 để nhận<color> <color=yellow>Phần Thưởng<color>");
	else
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5, "boss_hackylan");
	pTeamPlayer.AddExp(30000000)
		pTeamPlayer.AddStackItem(18,1,553,1,tbItemInfo,100) --tien du long
			pTeamPlayer.Earn(1000000,0);
		pTeamPlayer.AddBindMoney(1000000);
		pTeamPlayer.AddBindCoin(100000);--5v ??ng khóa
		pTeamPlayer.Msg("<color=green>Hành trang trống >1 để nhận<color> <color=yellow>Phần Thưởng<color>");
			end
		end
	end
...
local szMsg = string.format("Tổ đội của <color=green>%s<color> tại <color=green>"..GetMapNameFormId(nMapId).."<color> hạ gục <color=green>Hắc Kỳ Lân<color>!!!", pPlayer.szName);
GlobalExcute({"Dialog:GlobalNewsMsg_GS","<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ gục <color=green>Hắc Kỳ Lân<color> "});
	KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ gục <color=green>Hắc Kỳ Lân<color> ");
	KDialog.MsgToGlobal("<color=red>[Anh Hùng]<color> <color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ gục <color=green>Hắc Kỳ Lân<color> ");	
	KDialog.NewsMsg(0, Env.NEWSMSG_COUNT, szMsg);
	self:BroadCast(szMsg);
		local nNpcMapId, nNpcPosX, nNpcPosY = him.GetWorldPos();
	KNpc.Add2(9650, 1, 0, nNpcMapId, nNpcPosX, nNpcPosY)
KItem.AddItemInPos(nNpcMapId,nNpcPosX,nNpcPosY,18,1,1,9);
...
KItem.AddItemInPos(nNpcMapId,nNpcPosX,nNpcPosY,18,1,2009,2);
KItem.AddItemInPos(nNpcMapId,nNpcPosX,nNpcPosY,18,1,2009,2);
KItem.AddItemInPos(nNpcMapId,nNpcPosX,nNpcPosY,18,1,2009,2);
KItem.AddItemInPos(nNpcMapId,nNpcPosX,nNpcPosY,18,1,2009,2);
KItem.AddItemInPos(nNpcMapId,nNpcPosX,nNpcPosY,18,1,2009,2);
...
KItem.AddItemInPos(nNpcMapId,nNpcPosX,nNpcPosY,18,1,2100,1);
KItem.AddItemInPos(nNpcMapId,nNpcPosX,nNpcPosY,18,1,2100,1);
...
function tbQinshihuangBoss:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 80 then
		...
		if Boss.Qinshihuang:GetBossStep() == 0 then
			...
			pNpc.SendChat(szMsg);
			Boss.Qinshihuang:Broadcast("Hắc Kỳ Lân nói: "..szMsg);
