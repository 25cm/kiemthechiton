...
-- 秦始皇boss
local tbQinshihuangBoss	= Npc:GetClass("boss_lamkylan");
...
function tbQinshihuangBoss:DeathLoseItem(tbLoseItem)
	...
	local szMsg = "<color=green>Lam Kỳ Lân rơi vật phẩm:<color>\n";
	local tbList = {};
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5, "boss_lamkylan");
	pPlayer.AddExp(30000000)
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5, "boss_lamkylan");
...
local szMsg = string.format("Tổ đội của <color=green>%s<color> tại <color=green>"..GetMapNameFormId(nMapId).."<color> hạ gục <color=green>Lam Kỳ Lân<color>!!!", pPlayer.szName);
GlobalExcute({"Dialog:GlobalNewsMsg_GS","<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ gục <color=green>Lam Kỳ Lân<color> "});
	KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ gục <color=green>Lam Kỳ Lân<color> ");
	KDialog.MsgToGlobal("<color=red>[Anh Hùng]<color> <color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ gục <color=green>Lam Kỳ Lân<color> ");	
	KDialog.NewsMsg(0, Env.NEWSMSG_COUNT, szMsg);
	self:BroadCast(szMsg);
		local nNpcMapId, nNpcPosX, nNpcPosY = him.GetWorldPos();
	KNpc.Add2(9649, 1, 0, nNpcMapId, nNpcPosX, nNpcPosY)
...
function tbQinshihuangBoss:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 80 then
		...
		if Boss.Qinshihuang:GetBossStep() == 0 then
			...
			pNpc.SendChat(szMsg);
			Boss.Qinshihuang:Broadcast("Lam Kỳ Lân nói: "..szMsg);
