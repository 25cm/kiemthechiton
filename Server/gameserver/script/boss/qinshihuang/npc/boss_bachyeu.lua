...
-- 秦始皇boss
local tbQinshihuangBoss	= Npc:GetClass("boss_bachyeu");
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5, "boss_bachyeu");
	pPlayer.AddExp(30000000)
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5, "boss_bachyeu");
	...
	local szMsg = string.format("Tổ đội của <color=green>%s<color> tại <color=green>"..GetMapNameFormId(nMapId).."<color> đánh gục <color=cyan>Siêu Boss<color>!!", pPlayer.szName);
GlobalExcute({"Dialog:GlobalNewsMsg_GS","<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ gục <color=cyan>Siêu Boss<color>"});
	KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ gục <color=cyan>Siêu Boss<color>");
	KDialog.MsgToGlobal("<color=red>[Anh Hùng]<color> <color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ gục <color=cyan>Siêu Boss<color>");	
...
function tbQinshihuangBoss:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 80 then
		...
		if Boss.Qinshihuang:GetBossStep() == 0 then
			...
			pNpc.SendChat(szMsg);
			Boss.Qinshihuang:Broadcast("Bạch Yêu nói: "..szMsg);
