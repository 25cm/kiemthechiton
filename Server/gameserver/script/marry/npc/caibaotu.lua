...
tbNpc.CAIBAOTU_NPC_ID = 30003					-- 财宝兔npcid
...
tbNpc.LIVETIME = 240;		-- 财宝兔的存在时间15秒
tbNpc.TIME_READLINE = 5;	-- 读条时间5秒
...
-- 对应4个不同档次婚礼的财宝兔数量
tbNpc.TB_COUNT_CAIBAOTU = {10, 15, 20, 25};

-- 对应4个不同档次婚礼的游戏进行多少次（进行几轮刷财宝兔）
tbNpc.TB_TIMES_CALLNPC = {10, 10, 10, 10};
...
function tbNpc:NextTime(nMapId, nEndTime, tbPosList)
	...
	local nWeddingLevel = Marry:GetWeddingLevel(nMapId);
	local nCallNpcCount = self.TB_COUNT_CAIBAOTU[nWeddingLevel];
...
function tbNpc:EndGame(nMapId)
	...
	local szMsg = "Không thể tìm thấy thỏ ngọc trong buổi lễ.";
...
function tbNpc:OnDialog()
	...
	GeneralProcess:StartProcess("Đang bắt ...", self.TIME_READLINE * Env.GAME_FPS,
