...
function League:RepairMemberLeague(nType, szLeagueName, szMemberName)
	...
	if (MODULE_GAMESERVER) then
		...
		if (pPlayer) then
			pPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_JOINSPORT, string.format("Sửa lỗi dữ liệu đội: Tên đội: %s, Tên người chơi: %s, Loại: %d", szLeagueName, szMemberName, nType));
		end
		Dbg:WriteLogEx(Dbg.LOG_INFO, "League", "RepairMemberLeague", szMemberName, szLeagueName, nType, "Sửa lỗi dữ liệu nhóm");
	end
end
...
function League:_DebugGetMemberList(nType, szLeagueName)
	local pLeague = self:FindLeague(nType, szLeagueName);
	if not pLeague then
		print(szLeagueName,"Đội không tồn tại");
		return
	end
