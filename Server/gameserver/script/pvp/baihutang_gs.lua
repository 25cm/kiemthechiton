...
function BaiHuTang:ShowTimeInfo(pPlayer, IsPK)
	...
	if (IsPK == 1) then
		...
		szMsgFormat = "<color=green>Hoạt động bắt đầu sau：<color><color=white>%s<color>";
	end
...
function BaiHuTang:AddFriendFavor(pSelf, tbPlayerList, nFavor)
	...
	for _, pPlayer in ipairs(tbPlayerList) do
		if (pPlayer.nId ~= pSelf.nId and 
			...
			pSelf.Msg(string.format("Độ thân mật của bạn và <color=yellow>%s<color> tăng %d điểm.", pPlayer.szName, nFavor * 2));
		end
	end
end
