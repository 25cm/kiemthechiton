...
function tbBaseFactionRest:StartRest()	--开启活动
	if self.nState > 0 then
		print("Hoạt động đang mở, vui lòng mở lại sau khi kết thúc. Nó không thể được khởi động lại.");
		return 0;
	end
...
function tbBaseFactionRest:CallRandomNpc()
	...
	for nPos=1, self.POS_SUM do
		...
		if pNPC == nil then
			print("Hoạt động phòng nghỉ thi đấu võ thuật, triệu hồi npc thất bại.");
			Dbg:WriteLog("Hoạt động trong phòng nghỉ thi đấu võ thuật "," Không thể tắt bộ hẹn giờ theo cách thủ công");
		else
...
function tbBaseFactionRest:OpenSingleShowMsg(pPlayer)	--打开单个玩家界面
	...
	if self.nState > 0 then
		local szMsgFormat = "<color=green>Thời gian tìm cờ còn: <color><color=white>%s<color>\n\n";
		local nLastFrameTime = tonumber(self.RESTSTATE[self.nState].nEndTime * Env.GAME_FPS);
		szMsgFormat = szMsgFormat .. string.format("<color=green>Cờ sẽ biến mất sau : %2s<color>", self.nState) .. "<color=white>%s<color>";
	...
	elseif self.nState == 0 then
		...
		local szMsgFormat = "<color=green>Hoạt động chưa mở: <color><color=white>%s<color>";
...
function tbBaseFactionRest:HmCloseRest()	--手动关闭
	if self.nState == 0 then
		print("Bộ hẹn giờ đã được tắt cho các sự kiện trong phòng nghỉ võ thuật, không thể tắt bộ hẹn giờ.");
		Dbg:WriteLog("Hoạt động trong phòng nghỉ thi đấu võ thuật "," Không thể tắt bộ hẹn giờ theo cách thủ công");
		return 0;
	end
	self:BaseCloseRest();
