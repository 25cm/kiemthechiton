...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		if nTeamId == 0 then
			if nFlag ~= 1 then
				pPlayer.AddKinReputeEntry(nWeiWang, "bosstdlt");
		...
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
					if nFlag ~= 1 then
						pTeamPlayer.AddKinReputeEntry(nWeiWang, "bosstdlt");
							pTeamPlayer.AddExp(1000000)
		...
		local szMsg = "Hảo hữu của bạn ["..pPlayer.szName.."] đã đánh bại "..him.szName.."。";
		pPlayer.AddJbCoin(100000);
		pPlayer.Msg("<color=blue>Bạn đã tiêu diệt Boss TDLT nhận được:<color> <color=yellow>10 vạn đồng thường<color>");
		pPlayer.SendMsgToFriend(szMsg);
		Player:SendMsgToKinOrTong(pPlayer, " đã đánh bại "..him.szName.."。", 0);
		------------------------------------------------------------
				GlobalExcute({"Dialog:GlobalNewsMsg_GS","Người chơi <color=green>"..pPlayer.szName.."<color> trong trận chiến Tranh Đoạt Lãnh Thổ đã hạ sát <color=red>"..him.szName.."<color> nhận được <color=gold>10 Vạn Đồng<color>"});
		KDialog.MsgToGlobal("Người chơi <color=green>"..pPlayer.szName.."<color> trong trận chiến Tranh Đoạt Lãnh Thổ đã hạ sát <color=gold>"..him.szName.."<color> nhận được <color=yellow>10 Vạn Đồng<color>");
-------------------------------------------------------------
		local szTongName = "Vô";
...
-- 世界唯Boss
local tbbosstdlt	= Npc:GetClass("bosstdlt");
tbbosstdlt._tbBase = tbWorldBossBase;

function tbbosstdlt:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbbosstdltCallOut[him.nTemplateId] = nil;
