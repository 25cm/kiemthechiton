...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		if nTeamId == 0 then
			if nFlag ~= 1 then
				pPlayer.AddKinReputeEntry(nWeiWang, "tanthuyhoang");
		...
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");		
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
					if nFlag ~= 1 then
						pTeamPlayer.AddKinReputeEntry(nWeiWang, "tanthuyhoang");
							pTeamPlayer.AddExp(50000000)
		pTeamPlayer.AddBindMoney(1000000);
		pTeamPlayer.AddBindCoin(50000);--5v ??ng khóa
		pTeamPlayer.AddStackItem(18,1,1194,21,tbItemInfo,1);
		...
		pPlayer.AddJbCoin(200000);
		pPlayer.AddStackItem(18,1,666,1,tbItemInfo,2);
		pPlayer.AddStackItem(18,1,1331,3,tbItemInfo,1);
						local tbLocalPlayer = KPlayer.GetAllPlayer();
	for _, pPlayer in pairs(tbLocalPlayer) do
		pPlayer.AddStackItem(18,1,1194,24,tbItemInfo,1);
		pPlayer.AddBindCoin(100000);--5v ??ng khóa
	end	
		pPlayer.Msg("<color=blue>Bạn đã tiêu diệt Boss Tần Thủy Hoàng nhận được:<color> <color=yellow>20 vạn đồng thường<color>");
		pPlayer.SendMsgToFriend(szMsg);
		Player:SendMsgToKinOrTong(pPlayer, " đã đánh bại võ lâm cao thủ "..him.szName.."。", 0);
		
		GlobalExcute({"Dialog:GlobalNewsMsg_GS","Người chơi <color=green>"..pPlayer.szName.."<color> tại Tầng 5 Tần Lăng đã đánh bại <color=red>Tần Thủy Hoàng<color> nhận được <color=blue>20 Vạn Đồng<color>!"});
KDialog.MsgToGlobal("Người chơi <color=green>"..pPlayer.szName.."<color> tại Tầng 5 Tần Lăng đã đánh bại <color=yellow>Tần Thủy Hoàng<color> nhận được <color=blue>20 Vạn Đồng<color>!");
...
-- 世界唯Boss
local tbtanthuyhoang	= Npc:GetClass("tanthuyhoang");
tbtanthuyhoang._tbBase = tbWorldBossBase;

function tbtanthuyhoang:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbtanthuyhoangCallOut[him.nTemplateId] = nil;
