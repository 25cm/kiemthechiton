...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		if nTeamId == 0 then
			if nFlag ~= 1 then
				pPlayer.AddKinReputeEntry(nWeiWang, "daituong");
		pPlayer.AddExp(5000000)
		pPlayer.AddBindMoney(300000);
		pPlayer.AddBindCoin(30000);--5v ??ng khóa
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				pTeamPlayer.AddKinReputeEntry(nWeiWang, "UniqueBoss");
							pTeamPlayer.AddExp(5000000)
		pTeamPlayer.AddBindMoney(300000);
		pTeamPlayer.AddBindCoin(30000);--5v ??ng khóa
		...
		local szMsg = "Hảo hữu của bạn ["..pPlayer.szName.."] đã giết chết "..him.szName.."。";
		pPlayer.AddJbCoin(30000);
		pPlayer.Msg("<color=blue>Bạn đã tiêu diệt Đại Tướng nhận được:<color> <color=yellow>3 vạn đồng thường<color>");
		-------------------------------------
		GlobalExcute({"Dialog:GlobalNewsMsg_GS","Người chơi <color=green>"..pPlayer.szName.."<color> trên chiến trường Tống Kim đã tiêu diệt <color=red>Đại Tướng<color> nhận được <color=gold>3 Vạn Đồng<color>"});
		KDialog.MsgToGlobal("Người chơi <color=green>"..pPlayer.szName.."<color> trên chiến trường Tống Kim đã tiêu diệt <color=gold>Đại Tướng<color> nhận được <color=yellow>3 Vạn Đồng<color>");
		--------------------------------------
		pPlayer.SendMsgToFriend(szMsg);
		Player:SendMsgToKinOrTong(pPlayer, " đã tiêu diệt "..him.szName.."。", 0);
...
-- 世界唯Boss
local tbdaituong	= Npc:GetClass("daituong");
tbdaituong._tbBase = tbWorldBossBase;

function tbdaituong:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbdaituongCallOut[him.nTemplateId] = nil;
