...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
					if nFlag ~= 1 then
						pTeamPlayer.AddKinReputeEntry(nWeiWang, "hoakylan1");
							pTeamPlayer.AddExp(50000000)
		pTeamPlayer.AddBindMoney(1000000);
		pTeamPlayer.AddBindCoin(100000);--5v ??ng khóa
		pTeamPlayer.AddStackItem(18,1,1194,22,tbItemInfo,1)	 ---nhht
		...
		--pPlayer.AddStackItem(18,1,1194,22,tbItemInfo,1); --tien du long
		pPlayer.AddJbCoin(300000);
		pPlayer.AddStackItem(18,1,666,2,tbItemInfo,1);
        pPlayer.AddStackItem(18,1,1331,3,tbItemInfo,2)	 ---nhht
				local tbLocalPlayer = KPlayer.GetAllPlayer();
	for _, pPlayer in pairs(tbLocalPlayer) do
		pPlayer.AddStackItem(18,1,1194,25,tbItemInfo,1);
		pPlayer.AddBindCoin(150000);--5v ??ng khóa
	end	
		pPlayer.Msg("Bạn đã tiêu diệt <color=blue>Boss<color> nhận được: <color=yellow>30 vạn đồng thường<color>");
		pPlayer.SendMsgToFriend(szMsg);
		Player:SendMsgToKinOrTong(pPlayer, " đã đánh bại võ lâm cao thủ "..him.szName.."。", 0);
		
		KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"Người chơi <color=green>"..pPlayer.szName.."<color> tại Tàn Tích Cung A Phòng đã đánh tiêu diệt <color=red>Hỏa Kỳ Lân<color> nhận được <color=blue>30 Vạn Đồng<color>");
        KDialog.MsgToGlobal("Người chơi <color=green>"..pPlayer.szName.."<color> tại Tàn Tích Cung A Phòng đã đánh tiêu diệt <color=yellow>Hỏa Kỳ Lân<color> nhận được <color=blue>30 Vạn Đồng<color>");
...
-- 世界唯Boss
local tbhoakylan1	= Npc:GetClass("hoakylan1");
tbhoakylan1._tbBase = tbWorldBossBase;

function tbhoakylan1:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbhoakylan1CallOut[him.nTemplateId] = nil;
