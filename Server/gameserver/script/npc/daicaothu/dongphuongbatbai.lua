...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		if nTeamId == 0 then
			if nFlag ~= 1 then
				pPlayer.AddKinReputeEntry(nWeiWang, "dongphuongbatbai");
		...
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
					if nFlag ~= 1 then
						pTeamPlayer.AddKinReputeEntry(nWeiWang, "dongphuongbatbai");
							pTeamPlayer.AddExp(2000000)
		pTeamPlayer.AddBindMoney(200000);
		...
		pPlayer.Msg("Bạn đã tiêu diệt <color=blue>Boss 120<color> nhận được: <color=yellow>5 vạn đồng thường<color>");
		pPlayer.SendMsgToFriend(szMsg);
		Player:SendMsgToKinOrTong(pPlayer, " đã đánh bại võ lâm cao thủ "..him.szName.."。", 0);
		
		KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"Người chơi <color=green>"..pPlayer.szName.."<color> đã đánh bại [Kim Dung Truyện] <color=red>Đông Phương Bất Bại<color> nhận được <color=blue>5 Vạn Đồng<color>!");
        KDialog.MsgToGlobal("Người chơi <color=green>"..pPlayer.szName.."<color> đã đánh bại [Kim Dung Truyện] <color=yellow>Đông Phương Bất Bại<color> nhận được <color=blue>5 Vạn Đồng<color>!");
...
-- 世界唯Boss
local tbdongphuongbatbai	= Npc:GetClass("dongphuongbatbai");
tbdongphuongbatbai._tbBase = tbWorldBossBase;

function tbdongphuongbatbai:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbdongphuongbatbaiCallOut[him.nTemplateId] = nil;
