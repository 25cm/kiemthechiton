...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		pPlayer.Msg("Bạn đã tiêu diệt <color=blue>Boss<color> nhận được: <color=yellow>5 vạn đồng thường<color>");
		pPlayer.SendMsgToFriend(szMsg);
		Player:SendMsgToKinOrTong(pPlayer, " đã đánh bại võ lâm cao thủ "..him.szName.."。", 0);
		
		KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"Người chơi <color=green>"..pPlayer.szName.."<color> đã đánh bại [Kim Dung Truyện] <color=red>Độc Cô Cầu Bại<color> nhận được <color=blue>5 Vạn Đồng<color>");
        KDialog.MsgToGlobal("Người chơi <color=green>"..pPlayer.szName.."<color> đã đánh bại [Kim Dung Truyện] <color=yellow>Độc Cô Cầu Bại<color> nhận được <color=blue>5 Vạn Đồng<color>");
...
-- 世界唯Boss
local tbdoccocaubai	= Npc:GetClass("doccocaubai");
tbdoccocaubai._tbBase = tbWorldBossBase;

function tbdoccocaubai:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbdoccocaubaiCallOut[him.nTemplateId] = nil;
