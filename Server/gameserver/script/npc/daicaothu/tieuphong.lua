...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		if nTeamId == 0 then
			if nFlag ~= 1 then
				pPlayer.AddKinReputeEntry(nWeiWang, "tieuphong");
		self:AwardXinDe(pPlayer, 300000);
		pPlayer.AddExp(2000000)
		pPlayer.AddBindMoney(20000);
		...
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");	
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
					if nFlag ~= 1 then
						pTeamPlayer.AddKinReputeEntry(nWeiWang, "tieuphong");
		...
		KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"Người chơi <color=green>"..pPlayer.szName.."<color> đã đánh bại [Kim Dung Truyện] <color=red>Tiêu Phong<color> nhận được <color=blue>5 Vạn Đồng<color>!");
	KDialog.MsgToGlobal("Người chơi <color=green>"..pPlayer.szName.."<color> đã đánh bại [Kim Dung Truyện] <color=yellow>Tiêu Phong<color> nhận được <color=blue>5 Vạn Đồng<color>!");
...
-- 世界唯Boss
local tbtieuphong	= Npc:GetClass("tieuphong");
tbtieuphong._tbBase = tbWorldBossBase;

function tbtieuphong:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbtieuphongCallOut[him.nTemplateId] = nil;
