...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		if nTeamId == 0 then
			if nFlag ~= 1 then
				pPlayer.AddKinReputeEntry(nWeiWang, "vodanhthantang");
		...
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");		
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
					if nFlag ~= 1 then
						pTeamPlayer.AddKinReputeEntry(nWeiWang, "vodanhthantang");
		...
		KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"Người chơi <color=green>"..pPlayer.szName.."<color> đã đánh bại [Kim Dung Truyện] <color=red>Vô Danh Thần Tăng<color> nhận được <color=blue>5 Vạn Đồng<color>!");
        KDialog.MsgToGlobal("Người chơi <color=green>"..pPlayer.szName.."<color> đã đánh bại [Kim Dung Truyện] <color=yellow>Vô Danh Thần Tăng<color> nhận được <color=blue>5 Vạn Đồng<color>!");
...
-- 世界唯Boss
local tbvodanhthantang	= Npc:GetClass("vodanhthantang");
tbvodanhthantang._tbBase = tbWorldBossBase;

function tbvodanhthantang:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbvodanhthantangCallOut[him.nTemplateId] = nil;
