...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		if nTeamId == 0 then
			if nFlag ~= 1 then
				pPlayer.AddKinReputeEntry(nWeiWang, "hutruc");
		...
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
					if nFlag ~= 1 then
						pTeamPlayer.AddKinReputeEntry(nWeiWang, "hutruc");
		...
		KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"Người chơi <color=green>"..pPlayer.szName.."<color> đã đánh bại [Kim Dung Truyện] <color=red>Hư Trúc<color> nhận được <color=blue>5 Vạn Đồng<color>!");
	KDialog.MsgToGlobal("Người chơi <color=green>"..pPlayer.szName.."<color> đã đánh bại [Kim Dung Truyện] <color=yellow>Hư Trúc<color> nhận được <color=blue>5 Vạn Đồng<color>!");
...
-- 世界唯Boss
local tbhutruc	= Npc:GetClass("hutruc");
tbhutruc._tbBase = tbWorldBossBase;

function tbhutruc:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbhutrucCallOut[him.nTemplateId] = nil;
