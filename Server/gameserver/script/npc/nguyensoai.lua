...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		if nTeamId == 0 then
			if nFlag ~= 1 then
				pPlayer.AddKinReputeEntry(nWeiWang, "nguyensoai");
		...
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				pTeamPlayer.AddKinReputeEntry(nWeiWang, "nguyensoai");
		...
		pPlayer.Msg("<color=blue>Bạn đã tiêu diệt Nguyên Soái nhận được:<color> <color=yellow>10 vạn đồng thường<color>");
		pPlayer.SendMsgToFriend(szMsg);
		Player:SendMsgToKinOrTong(pPlayer, " đã tiêu diệt "..him.szName.."。", 0);
		-------------------------------------
		GlobalExcute({"Dialog:GlobalNewsMsg_GS","Người chơi <color=green>"..pPlayer.szName.."<color> trên chiến trường Tống Kim đã tiêu diệt <color=red>Nguyên Soái<color> nhận được <color=gold>10 Vạn Đồng<color>"});
		KDialog.MsgToGlobal("Người chơi <color=green>"..pPlayer.szName.."<color> trên chiến trường Tống Kim đã tiêu diệt <color=gold>Nguyên Soái<color> nhận được <color=yellow>10 Vạn Đồng<color>");
		---------------------------------------
		local szTongName = "Vô";
...
-- 世界唯Boss
local tbnguyensoai	= Npc:GetClass("nguyensoai");
tbnguyensoai._tbBase = tbWorldBossBase;

function tbnguyensoai:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbnguyensoaiCallOut[him.nTemplateId] = nil;
