...
function Domain:DlgJunXu()
    ...
    Nếu muốn nhận nhiều quân nhu hơn, phải mời Bang chủ đến thiết lập mức quân nhu trong thời gian tranh đoạt lãnh thổ (21:00~21:55). Sau khi thiết lập, bạn có thể nhận nhiều quân nhu hơn.
...
function Domain:DeclareWar_Confirm(nDomainId, nTongId)	
	...
	if  szDomainName then
		Dialog:Say("    Bạn có chắc chắn để chọn<color=yellow>"..self:GetDomainName(nDomainId)..
			"<color>Là mục tiêu tuyên chiến của Bang Hội trong cuộc chiến tranh giành lãnh thổ này？\n<color=yellow>Các Bang Hội đã tuyên chiến với lãnh thổ：\n<color><color=green>"..
			szDomainName.."<color>", tbDlg);
	else
		Dialog:Say("    Bạn có chắc chắn để chọn<color=yellow>"..self:GetDomainName(nDomainId).."<color>Là mục tiêu tuyên chiến của Bang Hội trong cuộc chiến tranh giành lãnh thổ này？", tbDlg);
	end
end
...
function Domain:DeclareWar_Intro()
	...
	if #tbToAttack == 0 then
		szSay = szSay..[[
		<color=yellow><enter>  Bang hội của bạn chưa tuyên chiến, bang chủ hoặc trưởng lão chiến tranh cần tuyên chiến trước 21:30 để xác định mục tiêu chinh chiến, nếu không sẽ không có quyền tấn công.<color>
		]]
	else
		local szDeclareInfo = "";
...
function Domain:SetJunXu(nType, nLevel, nNum, nConfirm)
	...
	if nCurState ~= self.PRE_BATTLE_STATE and nCurState ~= self.BATTLE_STATE then
		Dialog:Say("Chỉ trong thời kỳ tuyên chiến (21:00~21:55) mới có thể thiết lập ngạch quân nhu.");
		return 0;
	end
