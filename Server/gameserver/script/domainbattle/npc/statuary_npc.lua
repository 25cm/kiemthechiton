...
function tbNpc:OnDialog()
	...
	if (nEventType == Domain.tbStatuary.TYPE_EVENT_NORMAL) then
		szMsg = string.format("Triều đình khen ngợi<color=green>%s<color>Vì những đóng góp xuất sắc của ông trong nhiệm vụ bảo vệ lãnh thổ của Vương quốc Tống, bức tượng này đã được dựng lên。", szStatName);
	else
		szMsg = string.format("Triều đình khen ngợi<color=green>%s<color>Bức tượng này được dựng lên vì kết quả xuất sắc đạt được trong giải đấu liên máy chủ。", szStatName);
	end
	

	szMsg = string.format("%s\n\nTôn kính hiện tại：<color=yellow>%d<color>\n\nĐộ bền hiện tại của tượng：<color=yellow>%d/%d<color>\n", szMsg, nRevere, nEndure, self.MAX_ENDURE);
	...
	if (0 >= nEndure) then
		...
		szMsg = string.format("%s\nDo lâu ngày không được sửa chữa nên đã hư hỏng hoàn toàn ..., nếu<color=yellow>%d<color>Nếu nó không được sửa chữa trong vòng ngày, bức tượng sẽ được phục hồi vĩnh viễn。", szMsg, nDetDay);
