...
function RoomThief:ThiefFinish()
	if self.nThiefFinish ~= 1 then
		...
		self:DelNpc("thief");
		self:MovieDialog(-1, "Người định không bằng trời định, chính bị quan phủ bộ khoái bắt được!");
		self:UpdateMsg();
		self:CheckWin();
	end
end
...
function RoomThief:UpdateMsg()
	local szMsg = string.format("<color=%s>Tương phi tặc cản nhập quan phủ đích mai phục quyển <color>\n<color=%s> tìm ra trương đức hằng đoạt lại thanh hoa bình sứ<color>", 
