...
local tbRuleBase	= Battle:GetRuleClass(3, "Hộ Cờ");

-- 杀不同Npc获得的积分
tbRuleBase.BOUNS_KILL_NPC				= { 1, 1, 2, 2, 3, 3, 5, 5, 0, 0 };
-- 玩家护旗成功奖励积分
...
function tbRuleBase:AddFlag(nCampId)
	...
	local szMsg		= string.format("<color=yellow>%s<color>Lá cờ chiến đấu xuất hiện trên<color=green>(%d, %d)<color>。", Battle.NAME_CAMP[nCampId], tbPos[2] / 8, tbPos[3] / 16);
...
function tbRuleBase:DeletePlayerFlag(pPlayer)
	...
	if ((2 == self.tbMission.nState) and (1 == tbBattleInfo.bHaveFlag) and (tbBattleInfo.pPlayer.nId == tbBattleInfo.tbCamp.nPlayerIsFlag)) then
		...
		local szMsg		= string.format("%Lá cờ chiến đấu được thả trên (%d, %d)。", Battle.NAME_CAMP[nCampId], nPosX / 8, nPosY / 16);
		self.tbMission:BroadcastMsg(szMsg);
...
function tbRuleBase:GetKillNpcBoardMsg(nRankId, nNpcBouns, pNpc)
	local szMsg		= "";
	szMsg = string.format("Bạn đã giết %s", Battle.NAME_RANK[nRankId]);
	return szMsg, 0;
end
...
function tbRuleBase:ReachToFlagDes(nCampId)
	...
	GeneralProcess:StartProcess("Trong cờ đứng ...", 3 * Env.GAME_FPS, {self.ReachToFlagDesSuc, self, tbCamp, pPlayer.nId}, 
				...
				if (1 == self.nDuringProtect_TimeFlag) then
					...
					if (tbPos and tbPos[1]) then
						...
						if (nDx*nDx + nDy*nDy <= nDistanceSqure) then
							...
							szMsg		= string.format("你护旗有功，获得奖励<color=yellow>%d<color>积分。", nAddBouns);
							pPlayer.Msg(szMsg);
						end
					end
				end
...
function tbRuleBase:GiveFlagCampBouns(nCampId, tbBattleInfo)
	...
	local szMsg	= string.format("我军%s %s成功护送战旗到达目的地，你获得护旗奖励<color=yellow>%d<color>积分。", Battle.NAME_RANK[tbBattleInfo.nRank], tbBattleInfo.pPlayer.szName, nAddBouns);
	...
	for _, tbPlayer in pairs(tbPlayerInfoList) do
		if (tbPlayer and tbPlayer.pPlayer) then
			...
			if (((nDetX * nDetX + nDetY * nDetY) <= nDistance * nDistance) and tbPlayer.pPlayer.nId ~= nProtectPlayerId) then
				...
				local szMsg	= string.format("你协助护旗有功奖励<color=yellow>%d<color>积分。", nAddBouns);
