...
function tbMSBase:SendMsgOther(tbPlayerReaultList)
	local szMsg = "Hảo hữu của bạn [%s] xếp thử %d trong chiến trường Tống Kim.";
	for n, tbPlayer in pairs(tbPlayerReaultList) do
		...
		if (pPlayer) then
			...
			Player:SendMsgToKinOrTong(pPlayer, " xếp thử ".. n .." trong chiến trường Tống Kim.", 0);
		end		
	end
end
...
function tbMSBase:OnDeath(pKillerNpc)
	...
	if (pKillerPlayer) then
		...
		local szMsg	= string.format("%s %s <color=yellow>%s<color> đánh bại %s %s <color=yellow>%s<color>",
...
function tbMSBase:OnTimer_SignupEnd()
	...
	local szMsg	= string.format("Chiến trường Tống Kim đang diễn ra, %s chính thức bắt đầu!", self.szBattleName);
...
function tbMSBase:OnTimer_SignupMsg()
	...
	if (self.nSignUpMsgCount > 0) then
		...
		local szMsg		= string.format("Cuộc chiến chưa bắt đầu, còn <color=yellow>%d giây<color=yellow> nữa", nFrame / Env.GAME_FPS);
		self:BroadcastMsg(szMsg);
	else
		return 0;	-- 关闭Timer
	end
...
function tbMSBase:_WriteStaLog_Mix(tbPlayerList)
	local szLog = self:_GetStateBattleSeqKey() .. "_Hệ Thống" .. "\t số người tham gia";
	local nCount = 0;
