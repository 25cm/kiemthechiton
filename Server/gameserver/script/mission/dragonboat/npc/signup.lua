...
function tbNpc:OnProductBoat1(nSel, nItemId)
	...
	if nGenId <= 0 then
		Dialog:Say("你的龙舟本类型技能改造已完成，不能再进行改造。", {{"返回上层", self.OnProductBoatSel, self, nItemId},{"结束对话"}});
		return 0;
	end
...
function tbNpc:OnProductBoat2(nSel, nSelSkill, nItemId) 
	...
	if Esport.DragonBoat:CheckSkill(pItem, nSkillId) > 0 then
		Dialog:Say("你的龙舟已经改造了这个技能,不能同时改造同一技能", {{"继续改造这只龙舟", self.OnProductBoat1, self, nSel, nItemId},{"结束对话"}});
		return 0;
	end
...
function tbNpc:OnProductBoat3(nSel, nSelSkill, nItemId) 
	...
	Dialog:Say("你成功改造这只龙舟。", {{"继续改造",self.ProductBoat, self},{"结束对话"}});
end
