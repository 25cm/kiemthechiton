...
function tbMission:SetCountDownContext(pPlayer, nTime)
	...
	Dialog:SetBattleTimer(pPlayer, "<color=green>比赛总时间倒计时：<color=white>%s<color>\n<color=green>比赛开始倒计时：<color=white>%s<color>\n", self.tbGameSumTimer:GetRestTime(), nTime);
...
function tbMission:SetPlayContext(pPlayer, nTime)
	...
	Dialog:SetBattleTimer(pPlayer, "<color=green>比赛总时间倒计时：<color=white>%s<color>\n<color=green>本轮剩余时间：<color=white>%s<color>\n", self.tbGameSumTimer:GetRestTime(), nTime);
end
...
function tbMission:SetRestContext(pPlayer, nTime)
	...
	pPlayer.SetFightState(0);
	Dialog:SetBattleTimer(pPlayer, "<color=green>比赛总时间倒计时：<color=white>%s<color>\n\n<color=green>休息剩余时间：<color=white>%s<color>", self.tbGameSumTimer:GetRestTime(), nTime);
end
...
function tbMission:BlizzardControl()
	if self.nNpcBlizzardId then
		...
		if pNpc then
			for _, pPlayer in pairs(self:GetPlayerList()) do
				Dialog:SendBlackBoardMsg(pPlayer, "啊，好像要下冰雹了，小心被砸破头！");
			end
...
function tbMission:NianShouControl()
	...
	if(state == 1)then
		...
		self:DelRefreshNpc()
		for _, pPlayer in pairs(self:GetPlayerList()) do
			Dialog:SendBlackBoardMsg(pPlayer, "年兽：谁在下面这么吵，扰我清梦，找死！");
		end
	...
	elseif (state == 2) then
		for _, pPlayer in pairs(self:GetPlayerList()) do
			Dialog:SendBlackBoardMsg(pPlayer, "年兽：都躲哪去了，给老子滚出来！");
	...
	elseif (state == 4) then
		...
		for _, pPlayer in pairs(self:GetPlayerList()) do
			Dialog:SendBlackBoardMsg(pPlayer, "年兽：不给你们点颜色看看不知道厉害，老子继续去睡觉！");
		end
	end
