...
tbNpc.tbTypeName = {"Trò ch?i ném bóng tuy?t", "?ua thuy?n r?ng", "Ng??i b?o v? linh h?n c?a t? tiên"}


tbNpc.tbChangeItemList = {
	[1] = {
		szContext = "?á bóng 1 tháng ?? ??i l?y m?t ??i g?ng tay ?m áp",
		tbGiftParam = {
	...
	[2] = {
		szContext = "5 tháng ?á bóng ?? ??i l?y m?t ??i g?ng tay ?m · Yuxue",
		tbGiftParam = {
...
function tbNpc:OnDialog()
	local szMsg = string.format("Th? thao gia ?ình và các ho?t ??ng hàng ngày");

	local tbOpt = {
		{"Th? thao gia ?ình", self.OnDialogEx, self},
		{"<color=yellow>Ching Ming-B?o v? linh h?n c?a t? tiên<color>", TowerDefence.tbNpc.OnDialog, TowerDefence.tbNpc},
		{"K?t thúc"},
	};	
...
function tbNpc:OnDialogEx()
	...
	local nFlag = 0;
	local tbOpt = {{"T?i s? xem qua m?t chút"},};
	if (nState == EPlatForm.DEF_STATE_REST) then
		nFlag =1;
		tbOpt = {
			{"Nh?n ph?n th??ng s? ki?n cu?i cùng", EPlatForm.GetPlayerAward_Final, EPlatForm},
			{"Nh?n ph?n th??ng cho gia ?ình", EPlatForm.GetKinAward, EPlatForm},
			{"K?t thúc"},	
		};
	end
	if nFlag == 1 then
