...
Looker.TYPE = {
	...
	[1] = {{"Wlls:LookOnEnterReady"}, 	{"Wlls:LookOnLeaveReady"}, 	1428};
	[2] = {{"Wlls:LookOnEnterPk"}, 		{"Wlls:LookOnLeavePk"}, 	1428};
}

function Looker:Join(pPlayer, nType, nMapId, nPosX, nPosY, nLeaveType, tbLeavePos)
	if pPlayer.nFightState == 1 then
		pPlayer.Msg("Đang trong trạng thái chiến đấu không thể vào.");
...
function Looker:GetParamInt(nId)
	if nId <= 0 or nId > 10 then
		print("[Looker:GetParam] Thông số lỗi", "Chỉ có thể cho phép tối đa 10 thông số số nguyên.");
		return 0;
	end
...
function Looker:GetParamStr(nId)
	if nId <= 0 or nId > 10 then
		print("[Looker:GetParam] Thông số lỗi", "Chỉ có thể cho phép lên đến 10 thông số chuỗi.");
		return 0;
	end
