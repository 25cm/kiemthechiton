...
function Wlls:OnLookDialog(nReadyId, nSure)
	...
	local szMsg = string.format("现在正在进行<color=yellow>高级联赛%s强赛<color>。\n选择你想观看的联赛战队", nLevel);
	local tbOpt = {};
	if nLevel == 8 then
		for i=1, 4 do
...
function Wlls:LookOnEnterReady()
	...
	local szUiName1 = "<color=gray>无队伍<color>";
	local szUiName2 = "<color=gray>无队伍<color>";
	...
	Wlls:UpdateMsgUi(me, string.format("\n<color=green>观看对战战队：\n<color=gold>%s<color> Vs <color=gold>%s<color>\n\n<color=green>等待比赛开始<color>", szUiName1, szUiName2));
	Dialog:SendBlackBoardMsg(me, "你将要观看联赛比赛，准备时间结束后，比赛将自动开始。")
	me.Msg("进入武林联赛准备场，<color=yellow>准备时间结束后<color>，你会<color=yellow>自动进入比赛场<color>观看比赛。");
