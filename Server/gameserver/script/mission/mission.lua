	...
	function tbTimer:OnDestroy(nRegisterId)
		...
		if (tbTimers) then	-- 如果tbTimers不存在说明此Mission已经关闭了（OnDestroy是有可能延后的）
			if (tbTimers[nRegisterId] ~= self) then
				print(debug.traceback("Mission Error: Mission Đã đóng cửa"));
			end
...
function tbBase:JoinPlayer(pPlayer, nGroupId)
	...
	if self.tbPlayers[nPlayerId] then
		print(debug.traceback("Mission Error: Người chơi đã tham gia"));
		return 0;
	end
...
function tbBase:ChangePlayerGroup(pPlayer, nNewGroupId)
	...
	if not tbPlayerInfo then
		print("Mission Error: Chưa tham gia Mission", pPlayer.szName);
		print(debug.traceback("Mission Error: Chưa tham gia Mission"));
		return 0;
	end
...
function tbBase:GetPlayerList(nGroupId)
	...
	for nIndex, nPlayerId in ipairs(tbGroup) do
		...
		if pPlayer then
			...
			print(debug.traceback("Mission Error: Người chơi không tồn tại"));
		end
	end
...
function tbBase:_RemoveGroupPlayer(nGroupId, nPlayerId)
	local tbGroup	= self.tbGroups[nGroupId];
	if not tbGroup then
		print(debug.traceback("Mission Error:Group Không tồn tại"));
		return 0;
	end
	...
	print(debug.traceback("Mission Error: Không tìm thấy nPlayerId phù hợp"));
