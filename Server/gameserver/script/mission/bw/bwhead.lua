...
function BiWu:OnShowKey(nMapId)
	...
	if (nMapId == 0) then
		Dialog:Say(string.format("%s：对不起，你不是本次擂台的擂主之一，我不能告诉你入场号码。", him.szName), {{"结束对话",self.OnCancel}});
		return 0;
	end;
	...
	if (self.tbMission[nMapId].nType <= 1) then
		Dialog:Say(string.format("%s：你申请的比武擂台是1对1的比赛，不需要知道入场号码，你可以直接进场比赛。", him.szName), {{"结束对话",self.OnCancel}});
		return 0;
	end
	...
	Dialog:Say(string.format("%s：你的选手的入场号码为：<color=red>[%d]<color>，请把该号码通知你的参赛队员，队员只有输入正确该号码才能入场参加比赛，谢谢。", him.szName, nKey), {{"结束对话",self.OnCancel}});
end
