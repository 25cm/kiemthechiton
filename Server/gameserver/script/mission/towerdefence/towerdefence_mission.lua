...
tbMission.tbFirst_Title 					= {6, 21, 1, 0};		----称号奖励：守护精英（即时第一名）
...
tbMission.NPC_TYPE			= {	[6682] = {"小怪", 1},
								[6698] = {"小怪", 1},
								[6701] = {"小怪", 1},
								[6704] = {"小怪", 1},
								[6683] = {"<color=blue>精英<color>", 2},
								[6699] = {"<color=blue>精英<color>", 2},
								[6702] = {"<color=blue>精英<color>", 2},
								[6705] = {"<color=blue>精英<color>", 2},
								[6684] = {"<color=purple>首领<color>",3}, 
								[6700] = {"<color=purple>首领<color>",3},
								[6703] = {"<color=purple>首领<color>",3},
								[6706] = {"<color=purple>首领<color>",3},
...
function tbMission:SetEndContext(pPlayer, nTime)
	...
	Dialog:SetBattleTimer(pPlayer, "<color=green>比赛结束倒计时：<color=white>%s<color>\n", nTime);	
...
function tbMission:OnCountDown()	
	for _, pPlayer in pairs(self:GetPlayerList()) do		
		...
		Dialog:SendBlackBoardMsg(pPlayer, "怪物要进攻了，大家快做好准备啊！");
	end
...
function tbMission:SetPlayContext(pPlayer, nTime)
	...
	Dialog:SetBattleTimer(pPlayer, "<color=green>防守时间：<color=white>%s<color>\n", nTime);
end
...
function tbMission:OnEndPlay()
	self:BroadcastBlackBoardMsg(string.format("进攻结束！",nNumber));
	self:CloseAllTimer();
	self:ClearAllNpc();
	--刷新右边倒计时	
	for _, pPlayer in pairs(self:GetPlayerList()) do
...
function tbMission:SetCountDownContext(pPlayer, nTime)
	nTime = nTime or self.TIME_FPS_REST;	
	Dialog:SetBattleTimer(pPlayer, "\n<color=green>比赛开始倒计时：<color=white>%s<color>",  nTime);
...
function tbMission:GenerateAndSendMsg()
	...
	for _, tbGroupGrade in ipairs(tbGradeEx) do
		table.insert(tbMsg, string.format("<color=blue>%-16s<color><color=white>%d<color>", self.tbGroupCaptain[tbGroupGrade[1]].."队", tbGroupGrade[2]));
	end
	...
	if self.tbGrade_player[1] and self.szFirstName ~= self.tbGrade_player[1][1] and self.tbGrade_player[1][2] ~= 0 and self.nStateJour < 4  then
		...
		self:BroadcastBlackBoardMsg(string.format("第一名称号光环被%s夺走了，大家加紧追赶抢夺！",self.szFirstName));		
	end
	...
	for _, pPlayer in pairs(self:GetPlayerList())do
		local szMsgEx = szMsg..string.format("\n\n<color=yellow>自己的军饷：%s\n逃跑的怪物：%s<color>\n第%s波怪物", pPlayer.GetTask(TowerDefence.TSK_GROUP, TowerDefence.TSK_MONEY), self.nLostNpc, self.nResfreshNum);
...
function tbMission:UpdataMsg(nPlayerId, nGroupId)
	local pPlayer = KPlayer.GetPlayerObjById(nPlayerId);
	if pPlayer then
		Dialog:SendBattleMsg(pPlayer, string.format("\n<color=yellow>军饷：%s\n\n积分：%s\n\n逃跑的怪物：%s<color>", pPlayer.GetTask(TowerDefence.TSK_GROUP, TowerDefence.TSK_MONEY), self.tbGrade[nGroupId], self.nLostNpc));
	end
end
...
function tbMission:RefreshNpc(nNumber, nFlag)
	self:BroadcastBlackBoardMsg(string.format("第%s波怪物马上要进攻了！",nNumber));	
	if not nFlag then
		if nNumber ~= 1 then
	...
	if type(TowerDefence.NPC_TYPE_ID[nNumber]) == "table" then
		--不是boss波	
		self:BroadcastBlackBoardMsg(string.format("第%s波怪物开始进攻了！",nNumber));
...
function tbMission:AddMoney(nNumber)
	...
	self:BroadcastBlackBoardMsg("义军发军饷了，大家全力抗击！");
...
function tbMission:RefreshBoss(nNumber, nFlag, nNpcId)		
	local pNpc = nil;
	if nFlag == 1 then
		self:BroadcastBlackBoardMsg("一只疯狂的恶魔来了，快打死他，要不然他会一直破坏这里的！");		
...
function tbMission:AddTower(nPlayerId, nId, nItemId)
	...
	if pNpc then
		...
		if pPlayer.GetCurCamp() == 0 then
			pNpc.SetTitle(string.format("%s的蘑菇", pPlayer.szName));
		else
			pNpc.SetTitle(string.format("<color=gold>%s的蘑菇<color>", pPlayer.szName));
		end
...
function tbMission:UpDataTowerEx(nNpcId, nTypes)	
	...
	if pNpcEx then
		...
		if nCamp == 0 then
			pNpcEx.SetTitle(string.format("%s的蘑菇", pPlayer.szName));
		else
			pNpcEx.SetTitle(string.format("<color=gold>%s的蘑菇<color>", pPlayer.szName));
		end
