...
function TowerDefence:StartSignUp()
	...
	if MODULE_GAMESERVER then
		...
		KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, "守护先祖之魂开始接受报名，请60级以上玩家到各新手村报名参加");
	end	
	self:OnInit();
	self.nReadyState = 0;
...
function TowerDefence:AddHonor(szName, nHonor)
	...
	if MODULE_GAMESERVER then
		...
		if nPlayerId and nPlayerId > 0 then
			local pPlayer = KPlayer.GetPlayerObjById(nPlayerId);
			if pPlayer then
				pPlayer.Msg(string.format("恭喜您获得了<color=yellow>%s点<color>禅境花园荣誉点", nHonor));
			end
		end
		return 0;
	end
...
function TowerDefence:WriteLog(szLog, nPlayerId)
	if nPlayerId then
		local pPlayer = KPlayer.GetPlayerObjById(nPlayerId)
		if (pPlayer) then
			Dbg:WriteLog("TowerDefence", "守护先祖之魂", pPlayer.szAccount, pPlayer.szName, szLog);
			return 1;
		end
	end
	Dbg:WriteLog("TowerDefence","守护先祖之魂", szLog);
end
