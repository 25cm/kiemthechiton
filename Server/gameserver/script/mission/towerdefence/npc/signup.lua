...
function tbNpc:GetFinishAward()
	...
	local nRankType = 0;
	local szRank = string.format("排名为<color=yellow>第%s名<color>", nRank);
	if nRank <= 0 then
		szRank = "无排名";
	end
	if nRank > 0 then
...
function tbNpc:OnOpenGiftOk(tbItemObj)
	...
	me.Msg(string.format("您获得了<color=yellow>%s次<color>额外参赛资格。", nDelCount));
end
...
function tbNpc:OnAboutNewYears(nSel)
	if nSel then
		Dialog:Say(self.tbAboutNewYears[nSel], {{"返回上层", self.OnAboutNewYears, self}});
		return 0;
	end
