...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		if nTeamId == 0 then
			if nFlag ~= 1 then
				pPlayer.AddKinReputeEntry(nWeiWang, "quanmongco2");
		...
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
					if nFlag ~= 1 then
						pTeamPlayer.AddKinReputeEntry(nWeiWang, "quanmongco2");
		...
		pPlayer.Msg("<color=blue>Bạn đã tiêu diệt Boss nhận được:<color> <color=yellow>10 vạn đồng thường<color>");
		pPlayer.SendMsgToFriend(szMsg);
		Player:SendMsgToKinOrTong(pPlayer, " đã đánh bại võ lâm cao thủ "..him.szName.."。", 0);
		------------------------
		GlobalExcute({"Dialog:GlobalNewsMsg_GS","Người chơi<color=green>"..pPlayer.szName.."<color> trong trận chiến Chống Quân Mông Cổ đã tiêu diệt <color=red>"..him.szName.."<color> nhận được <color=gold>10 Vạn Đồng<color>"});
		KDialog.MsgToGlobal("Người chơi<color=green>"..pPlayer.szName.."<color> trong trận chiến Chống Quân Mông Cổ đã tiêu diệt <color=gold>"..him.szName.."<color> nhận được <color=yellow>10 Vạn Đồng<color>");
		-----------------------------------------
		local szTongName = "Vô";
...
-- 世界唯Boss
local tbquanmongco2	= Npc:GetClass("quanmongco2");
tbquanmongco2._tbBase = tbWorldBossBase;

function tbquanmongco2:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbquanmongco2CallOut[him.nTemplateId] = nil;
