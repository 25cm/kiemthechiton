...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		if nTeamId == 0 then
			if nFlag ~= 1 then
				pPlayer.AddKinReputeEntry(nWeiWang, "tonggiang1");
		...
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
					if nFlag ~= 1 then
						pTeamPlayer.AddKinReputeEntry(nWeiWang, "tonggiang1");
		...
		pPlayer.AddStackItem(18,1,1331,3,nil,1);
		pPlayer.AddJbCoin(50000);
		pPlayer.AddStackItem(18,1,1337,4,nil,5);
		pPlayer.Msg("<color=blue>Bạn đã tiêu diệt Boss nhận được:<color> <color=yellow>5 vạn đồng thường<color>");
		pPlayer.SendMsgToFriend(szMsg);
		Player:SendMsgToKinOrTong(pPlayer, " đã đánh bại võ lâm cao thủ "..him.szName.."。", 0);
		------------------------
		GlobalExcute({"Dialog:GlobalNewsMsg_GS","Người chơi<color=green>"..pPlayer.szName.."<color> trong trận chiến giải cứu [Xe Chở Lương Thảo] Triều Đình đã tiêu diệt <color=red>"..him.szName.."<color> nhận được <color=gold>5 Vạn Đồng<color>"});
		KDialog.MsgToGlobal("Người chơi<color=green>"..pPlayer.szName.."<color> trong trận chiến giải cứu [Xe Chở Lương Thảo] Triều Đình đã tiêu diệt <color=gold>"..him.szName.."<color> nhận được <color=yellow>5 Vạn Đồng<color>");
		-----------------------------------------
		local szTongName = "Vô";
...
-- 世界唯Boss
local tbtonggiang1	= Npc:GetClass("tonggiang1");
tbtonggiang1._tbBase = tbWorldBossBase;

function tbtonggiang1:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbtonggiang1bCallOut[him.nTemplateId] = nil;
