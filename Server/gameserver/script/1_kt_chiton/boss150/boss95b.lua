...
function tbWorldBossBase:OnDeath(pNpcKiller, nFlag)
	...
	if (pPlayer) then
		...
		if nTeamId == 0 then
			if nFlag ~= 1 then
				pPlayer.AddKinReputeEntry(nWeiWang, "boss95b");
		...
		pPlayer.Msg("<color=green>Hành trang trống >=5 để nhận<color> <color=yellow>Phần Thưởng<color>");
			...
			for i, nPlayerId in pairs(tbPlayerId) do
				...
				if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
					if nFlag ~= 1 then
						pTeamPlayer.AddKinReputeEntry(nWeiWang, "boss95b");
		...
		------------------------
		GlobalExcute({"Dialog:GlobalNewsMsg_GS","Người chơi<color=green>"..pPlayer.szName.."<color> đánh bại Võ Lâm Cao Thủ <color=red>"..him.szName.."<color> nhận được <color=gold>10 Vạn Đồng<color>"});
		KDialog.MsgToGlobal("Người chơi<color=green>"..pPlayer.szName.."<color> đánh bại Võ Lâm Cao Thủ <color=gold>"..him.szName.."<color> nhận được <color=yellow>10 Vạn Đồng<color>");
		-----------------------------------------
		local szTongName = "Vô";
...
-- 世界唯Boss
local tbboss95b	= Npc:GetClass("boss95b");
tbboss95b._tbBase = tbWorldBossBase;

function tbboss95b:OnDeath(pNpcKiller)
	self._tbBase:OnDeath(pNpcKiller);
	Boss.tbboss95bCallOut[him.nTemplateId] = nil;
