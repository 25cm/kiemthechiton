...
function Fourfold:OnDialog()
	...
	if nRemainTime <= 0 then
		Dialog:Say(string.format("%sThời gian tu luyện bí cảnh của ngươi đã hết, hãy quay lại vào ngày mai.", szMsgTilte))
		return 0;
	end
	if me.nTeamId <= 0 then
		Dialog:Say(string.format("%sNgươi phải <color=yellow>ở trong nhóm<color> và làm <color=yellow>đội trưởng<color>, mang theo bằng hữu của ngươi vào tu luyện.", szMsgTilte))
		return 0;
	end
...
function Fourfold:ApplyTeamMap(nFlag, nHour, nMapNumber)
	...
	if not nFlag then
		...
		for _, nHour in ipairs(tbHour) do
			table.insert(tbOpt, {string.format("Sử dụng %d bản đồ tu luyện %d giờ",nHour/2, nHour), self.ApplyTeamMap, self, 1, nHour, nHour/2});
		end
	...
	if self:ApplyMap(nMapId, me.nId, me.nLevel, nHour) == 1 then
		...
		me.Msg("<color=yellow>Mở Bí Cảnh thành công<color>, hãy gọi bằng hữu tới, trong thời gian <color=yellow>3 phút<color> vào Bí Cảnh tu luyện.");
		return 0;
	end
end
