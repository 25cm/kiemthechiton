...
function tbBoss_5:OnDeath(pNpc)
	...
	----------------boss-----------------------
		...
		pPlayer.Msg("Bạn đã tiêu diệt <color=green>Boss cuối Thiên Quỳnh Cung<color> nhận được: <color=yellow>10 vạn đồng thường<color>");
---------------------------------------------------
        GlobalExcute({"Dialog:GlobalNewsMsg_GS","Người chơi <color=green>"..pPlayer.szName.."<color> tiêu diệt Boss Tàng Bản Đồ <color=red>Thiên Quỳnh Cung<color> nhận được <color=gold>10 Vạn Đồng<color>"});
		KDialog.MsgToGlobal("Người chơi <color=green>"..pPlayer.szName.."<color> tiêu diệt Boss Tàng Bản Đồ <color=gold>Thiên Quỳnh Cung<color> nhận được <color=yellow>10 Vạn Đồng<color>");
	
	--------------------------------------
		pPlayer.DropRateItem(TreasureMap.tbDrop_Level_3["Npc_Boss2"], 28, -1, -1, him);
		TreasureMap:AwardWeiWangAndXinde(pPlayer, 2, 5, 1, 100000);
		    --KNpc.Add2(2597, 1, -1, nMapId, 1816, 2665);
		-- 添加亲密度
...
function tbBoss_6:OnDeath(pNpc)
	...
	pSendPos.szName	= "Lối đi bí mật trong Thiên Quỳnh Cung";
...
function tbBoss_1:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 50 and tbInstancing.tbBossLifePoint[nBossId][50] == 0 then
		...
		him.SendChat("Mau ra đây giúp ta!");
	end;
	if nLifePercent == 30 and tbInstancing.tbBossLifePoint[nBossId][30] == 0 then
		...
		him.SendChat("Người đâu, mau ra giúp ra!");
	end;	
end;
...
function tbBoss_2:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 50 and tbInstancing.tbBossLifePoint[nBossId][50] == 0 then
		...
		tbInstancing.tbBossLifePoint[nBossId][50] = 1;
		him.SendChat("Các ngươi còn chờ cái gì nữa?");
	end;
	if nLifePercent == 30 and tbInstancing.tbBossLifePoint[nBossId][30] == 0 then
		for i=1, #tbNpcPos[30] do
			KNpc.Add2(2732, 80, -1, nMapId, tbNpcPos[30][i][1], tbNpcPos[30][i][2]);
		...
		tbInstancing.tbBossLifePoint[nBossId][30] = 1;
		him.SendChat("Mau ra đây, bảo vệ ta!");
	end;	
end;
...
function tbBoss_3:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 50 and tbInstancing.tbBossLifePoint[nBossId][50] == 0 then
		...
		tbInstancing.tbBossLifePoint[nBossId][50] = 1;
		him.SendChat("Đến đây, cung nữ của ta!");
	end;
	if nLifePercent == 30 and tbInstancing.tbBossLifePoint[nBossId][30] == 0 then
		for i=1, #tbNpcPos[30] do
			KNpc.Add2(2732, 80, -1, nMapId, tbNpcPos[30][i][1], tbNpcPos[30][i][2]);
...
function tbBoss_4:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 50 and tbInstancing.tbBossLifePoint[nBossId][50] == 0 then
		...
		tbInstancing.tbBossLifePoint[nBossId][50] = 1;
		him.SendChat("Đánh đuổi lũ xâm lăng không cho chúng thoát!");
	end;
	if nLifePercent == 30 and tbInstancing.tbBossLifePoint[nBossId][30] == 0 then
		for i=1, #tbNpcPos[30] do
...
function tbBoss_5:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 50 and tbInstancing.tbBossLifePoint[nBossId][50] == 0 then
		...
		tbInstancing.tbBossLifePoint[nBossId][50] = 1;
		him.SendChat("Chữa trị cho ta, những bông hoa đẹp!");
	end;
	if nLifePercent == 30 and tbInstancing.tbBossLifePoint[nBossId][30] == 0 then
		for i=1, #tbNpcPos[30] do
		...
		tbInstancing.tbBossLifePoint[nBossId][30] = 1;
		him.SendChat("Nở ra đi! Bông hoa của ta!");
	end;	
end;
