...
function BatchArrest:CheckOverdue()
	...
	for szName, tbProcess in pairs(self.tbProcessList) do
		if (tbProcess.nStartTime < nPassTime) then
			...
			local szMsg	= string.format("[%s]Lô hàng[%s]Hoạt động đã bị hủy bỏ do hết thời gian。",
				os.date("%Y-%m-%d %H:%M:%S", tbProcess.nStartTime), (tbProcess.bArrest == 1 and "Nhà tù Quan Thiên") or "Giải phóng nhà tù");
...
function BatchArrest:ProcessPlayer(pPlayer)
	...
	local szMsg	= string.format("[%s]Lô hàng[%s]Hoạt động được thực hiện。", os.date("%Y-%m-%d %H:%M:%S", tbProcess.nStartTime), (tbProcess.bArrest == 1 and "Nhà tù Quan Thiên") or "Giải phóng nhà tù")
