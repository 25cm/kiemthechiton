...
function tb:ObtainMemberPos(nMemberPlayerId, nPlayerId, nItemId, nMapId, nPosX, nPosY, nFightState)
	...
	if nMapId == -1 then
		pPlayer.Msg("Kh?ng th? d?ch chuy?n ??n b?n ?? m?c tiêu！");
		return 0;
	end
	...
	if self:CheckMember(nPlayerId,nMemberPlayerId) ~= 1 then
		pPlayer.Msg("B?n kh?ng có ??ng ??i, có th? là ??ng ??i ?? r?i kh?i ??i！");
		return 0;		
	end
	-- 执行传送
	local pItem = KItem.GetObjById(nItemId);
	if pItem == nil then
		pPlayer.Msg("Kh?ng tìm th?y bi?u t??ng v? tr?, ho?t ??ng b?t h?p pháp. Vui lòng liên h? v?i GM.")
		return 0;
	end
	...
	if self.tbItemList[pItem.nParticular] - nUseCount == 1 then
		if (pPlayer.DelItem(pItem, Player.emKLOSEITEM_USE) ~= 1) then
			pPlayer.Msg("Kh?ng xóa ???c Bùa h? m?nh trong v? tr?!");
			return 0;
		end
	else
