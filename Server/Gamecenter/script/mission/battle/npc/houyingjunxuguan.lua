...
function tbJunXu:OnDialog(szCamp)
	...
	if not GLOBAL_AGENT then
		tbOpt = {
			...
			{"Mua đạo cụ chiến trường", tbBaoJunXu.OnGetJunXuMed, tbBaoJunXu},
			{"Trở lại điểm báo danh", self.OnLeaveSay, self},
			{"Kết thúc đối thoại"},
		};
	else
		tbOpt = {
			{"Mua đạo cụ", self.OnBuyWldhYao, self},
			{"Vào chiến trường", self.OnLeaveHere, self},
			{"Kết thúc đối thoại"},
		};
	end
...
function tbJunXu:OnLeaveSay()
	...
	local tbOpt = {
				{"Đồng Ý", self.OnLeave, self,him.dwId},
				{"Kết thúc đối thoại"},
		};
	Dialog:Say("Ngươi có chắc muốn quay trở lại điểm báo danh? ", tbOpt);
end
