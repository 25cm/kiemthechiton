...
function tbCampBase:_BaseToOuter(pPlayer, szPosName)
	...
	if (nRemainTime > 0) then
		Dialog:Say(string.format("Hãy nghỉ ngơi trong trại sau %d giây, và sau đó đi tiêu diệt kẻ thù sau khi bạn đã chuẩn bị đầy đủ。", nRemainTime));
		return;
	end
...
function tbCampBase:OnTrap_OuterToBase(pPlayer)
	...
	if (tbBattleInfo.tbCamp.nCampId ~= self.nCampId) then
		tbBattleInfo.pPlayer.Msg("Phía trước được xếp bằng giáo và dây xích. Quân lính canh gác cẩn mật. Nếu có binh lính hạng nặng, bạn vẫn không muốn lao vào.！");
		return;
	end
...
function tbCampBase:_OuterToField(pPlayer, szPosName)
	...
	if (2 ~= nState) then
		Dialog:Say("Trước khi lâm trận, tất cả binh lính không được phép rời trại khi chưa được phép, các bạn nên đợi khi lâm trận hãy dũng cảm đánh giặc.！");
