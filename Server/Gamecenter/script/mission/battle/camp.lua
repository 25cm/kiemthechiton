...
function tbCampBase:OnEnd(nResult, szWinMsg)	
	...
	for _, pPlayer in pairs(tbPlayerList) do
		...
		pPlayer.Msg(string.format("Xếp hạng cuối cùng của bạn là: <color=green> %d <color> và số điểm kiếm được là: <color=yellow> %d <color>", tbBattleInfo.nListRank, tbBattleInfo.nBouns));
	end
end
...
function tbCampBase:OnProtectFlag(tbFlagPlayer)
	...
	local szFirMsg = string.format("% s army% s <color = green>% s <color> đã hộ tống thành công lá cờ chiến đấu đến đích", Battle.NAME_CAMP[tbFlagPlayer.tbCamp.nCampId], Battle.NAME_RANK[tbFlagPlayer.nRank], tbFlagPlayer.pPlayer.szName);
	local szSecMsg = string.format("Hiện tại, số lượng cờ chiến được% s hộ tống thành công là: <color=red>%d<color>", Battle.NAME_CAMP[tbFlagPlayer.tbCamp.nCampId], tbFlagPlayer.tbCamp.nFlags);
