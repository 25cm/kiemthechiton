...
tbRuleBase.TIMER_ADD_MARSHAL	= Env.GAME_FPS * 60 * 25;	-- 出现元帅的最早时间
tbRuleBase.TIMER_ADD_DAJIANG	= Env.GAME_FPS * 60 * 20;	-- 出现大将的时间
tbRuleBase.TIMER_ADD_BOUNS		= Env.GAME_FPS * 60 * 0.5;	-- 护卫大将、元帅奖分间隔
tbRuleBase.TIMER_ADD_LongTru		= Env.GAME_FPS * 60 * 0.5;	-- 护卫大将、元帅奖分间隔
...
-- 杀不同Npc获得的积分
tbRuleBase.BOUNS_KILL_NPC	= { 1, 1, 2, 2, 3, 3, 5, 5, 50, 100 };
...
function tbRuleBase:OnStart()
	...
	self.tbMission:CreateTimer(self.TIMER_ADD_LongTru, self.OnTimer_AddLongTru, self);
	self.tbMission:CreateTimer(self.TIMER_ADD_BOUNS, self.OnTimer_AddBouns, self);
end
function tbRuleBase:OnTimer_AddLongTru()
	-- 已经加过元帅了
	
		-- local tbNpcObj		= self:AddFightNpc(nCampId, "Npc_longtru", tbRuleBase.RANKID_LONGTRUTL, 1);

	-- 刷元帅
	KNpc.Add2(7410,100,-1,1635,1807	,2951);
	KNpc.Add2(7410,100,-1,1636,1807	,2951);
	KNpc.Add2(7410,100,-1,1637,1807	,2951);
	KNpc.Add2(7410,100,-1,1638,1807	,2951);
	KNpc.Add2(7410,100,-1,1639,1807	,2951);
	KNpc.Add2(7410,100,-1,1640,1807	,2951);
	KNpc.Add2(7410,100,-1,1641,1807	,2951);
	KNpc.Add2(7410,100,-1,1642,1807	,2951);
	KNpc.Add2(7410,100,-1,1643,1807	,2951);
...
function tbRuleBase:GetKillNpcBoardMsg(nRankId, nNpcBouns, pNpc)
	...
	if (1 < nRankId and 9 > nRankId) then
		szMsg = string.format("Bạn giết %s", Battle.NAME_RANK[nRankId]);
	elseif (9 <= nRankId) then
		local szNpcName = pNpc.szName;
		nMidMsg = 2;
		szMsg 	= string.format("Xảy ra cuộc chiến đẫm máu, tiêu diệt <color=yellow>%s<color>, nhận được <color=yellow>%d<color> điểm thưởng.", szNpcName, nNpcBouns);
		if (9 == nRankId) then
			nMidMsg = 1;
		end
	end
...
function tbRuleBase:OnDeath_Yuanshuai(nCampId, pNpc)
	...
	if (szKillerName) then
		...
		local szMsg = string.format("<color=yellow>%s<color> quân <color=yellow>%s<color> anh dũng tiêu diệt <color=yellow>%s Nguyên Soái<color>, %s quân nhận được 10 điểm thưởng.", Battle.NAME_CAMP[nWinCampId], szKillerName, Battle.NAME_CAMP[nLoseCampId], Battle.NAME_CAMP[nWinCampId]);
...
function tbRuleBase:TryAddMarshal(nCampId, bForce)
	...
	local szMsg	= string.format("%s Nguyên Soái ra trận đánh giặc, %s quân mau chóng quay về, bảo vệ Nguyên Soái!", szCampName, szCampName);
	local tbPlayerList	= self.tbMission:GetPlayerList(nCampId);
	for _, pPlayer in pairs(tbPlayerList) do
		Dialog:SendInfoBoardMsg(pPlayer, szMsg);
	end
...
function tbRuleBase:OnTimer_AddDaJiang()
	for nCampId, tbMapInfo in pairs(self.tbMapInfoCamp) do		
		...
		local szCampName	= Battle.NAME_CAMP[nCampId];
		local szMsg			= string.format("%s Đại Tướng ra trận đánh giặc，%s quân mau chóng quay về, bảo vệ Đại Tướng!", szCampName, szCampName);
...
function tbRuleBase:AddProtectBouns(tbCamp, tbBoss, nRankId, nCampId, nDistanceSqure)
	...
	local szMsg		= string.format("Bảo vệ %s %s, nhận được <color=yellow>%d<color> điểm thưởng.",
...
function tbRuleBase:GiveKillBossCampBouns(nCampId, nRankId)
	...
	local szMsg	= string.format("%s %s bị quân ta tiêu diệt, nhận được <color=yellow>%d<color> điểm.",
						tbOppCamp.szCampName, Battle.NAME_RANK[nRankId], nAddBouns);
	if (10 == nRankId) then
		szMsg = szMsg .. "Điểm tích lũy tăng 10 điểm.";
	end
	self.tbMission:BroadcastMsg(nCampId, szMsg);
