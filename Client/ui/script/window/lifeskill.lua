...
function uiLifeSkill:WriteStatLog()
	Log:Ui_SendLog("Giao diện kỹ năng sống F8", 1);
end
...
function uiLifeSkill:PreOpen()
	if me.IsAccountLock() ~= 0 then
		...
		me.Msg("Tài khoản đang khóa, không thể sử dụng kỹ năng sống!");
		return 0;
	end
	return 1;
end
...
function uiLifeSkill:Update()
	...
	Txt_SetTxt(self.UIGROUP, TXT_MKP, "Tinh lực:"..me.dwCurMKP);
	Txt_SetTxt(self.UIGROUP, TXT_GTP, "Hoạt lực:"..me.dwCurGTP);
end
...
function uiLifeSkill:OnAddLifeSkill(nSkillId, nSkillLevel, nSkillCurExp)
	...
	Txt_SetTxt(self.UIGROUP, obSkill[4], "Cấp:"..me.GetSingleLifeSkill(nSkillId).nLevel);
	
	if nSkillId == 11 then
		Txt_SetTxt(self.UIGROUP, obSkill[4], "Cấp: - ");
	end;
end
