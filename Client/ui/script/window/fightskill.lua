...
function uiFightSkill:UpdateCommonSkill()
	...
	local dem = 0;
	for i=1,2000 do
		if me.GetSkillLevel(i) > 0 then
			dem = dem + 1
		end
	end

	if dem > 110 then
		Dialog:Say("Bạn đã học quá nhiều kỹ năng\n<color=green>Hãy pm Hỗ Trợ để được xóa đi những kỹ năng không còn cần thiết.<color>\n<color=red>Nếu không sẽ không xem được phần kỷ năng chung.<color>")
