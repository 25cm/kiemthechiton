		...
		if nCanJoin and  nCanJoin == 1 and nCountSuit ~= 0 and nMatchOpen == 1 then
			if not self.tbNotifyMatch[nIndexId] then
				for nTimeIndex, tbTimeInfo in ipairs(tbMatch.tbTime) do
					if (self:IsTimeNotify(nCurTime,tbTimeInfo[2]) == 1) then
				...
				if(self:IsTimeNotify(nCurTime,tbMatch.tbTime[nTimeIndex][2]) == 1) then
...
function tbCalendar:OnEnterGame()
    ...
    self.bMatchNotifyAll = tbSaveData:Load(self.DATA_KEY_2) or 0;
	self.tbNotifyMatch = {};
...
function tbCalendar:LoadTimingFile()
	...
	if not tbFile then
		print("【Calendar Error】 Đọc tập tin bị lỗi,LoadTimingFile", self.szTimingMatchPath);
		return;
	end	
	local tbTimingMatch = {};
...
function tbCalendar:LoadDailyFile()
	...
	if not tbFile then
		print("【Calendar Error】 Đọc tập tin bị lỗi,LoadDailyFile", self.szDailyMatchPath);
		return;
	end	
	local tbDailyMatch = {};
	...
	for i = 1, nLength do					
		...
		for j = 1, #tbOff do
			if tbOff[j] <= #(tbMatch[j].tbTime) then		
				if nMin == 0 or		
			  		(tbMatch[j].tbTime[tbOff[j]])[2] < (tbMatch[nMin].tbTime[tbOff[nMin]])[2] then -- 以开始时间排序
					nMin = j;
				end
			end
		end		
...
function tbCalendar:LoadMatchTime(szPath)
	...
	if not tbFile then
		print("【Calendar Error】 Đọc tập tin bị lỗi，LoadMatchTime",szPath);
		return;
	end
	
	for nId, tbParam in ipairs(tbFile) do
		if nId > 1 then
			table.insert(tbTime, {tonumber(tbParam.JoinTime),tonumber(tbParam.StartTime),tonumber(tbParam.EndTime)});	
		end
	end
	return tbTime;
end
...
function tbCalendar:LoadQA(szPath)
	...
	local tbFile = Lib:LoadTabFile(szPath);
	if not tbFile then
		print("【Calendar Error】 Đọc tập tin bị lỗi，LoadQA",szPath);
		return;
	end
...
function tbCalendar:LoadTimeFrame()
	...
	if not tbFile then
		print("【Calendar Error】 Đọc tập tin bị lỗi,LoadTimeFrame", self.szTimeFramePath);
		return;
	end	
	local tbTimeFrame = {};
	for nId, tbParam in ipairs(tbFile) do
