...
function Player:EventAllMap(him)
	...
	if (nMapId ~= 132) and (nMapId ~= 1536) and (nMapId ~= 1537) and (nMapId ~= 1538) and (nMapId ~= 1539) then
		local i = 0;
		local nAdd = 0;
		local nRand = 0;
		local nIndex = 0;
		...
		if (tbAward[nIndex]==1) then
			me.AddBindCoin(10);
	me.Msg("Nhận được <color=yellow>10 Đồng Khoá<color>");
		end;
		if (tbAward[nIndex]==2) then
		me.AddBindCoin(30);
	me.Msg("Nhận được <color=yellow>30 Đồng Khoá<color>");
		end;
	end;
end
