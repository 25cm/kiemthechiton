...
function tbTree:OnDialog()
	...
	if SpecialEvent.ZhiShu2009:IsBigTree(him) == 0 then
		...
		if SpecialEvent.ZhiShu2009:HasSeed(him) == 0 then
			...
			local szMsg = string.format("恩，看上去这棵树已经长大了，结满了果实，似乎可以摘取了！不过，好像再过%s也就要老去了。", Lib:TimeDesc(nDelta));
			local tbOpt = {
				{"摘取果实", self.GatherSeed, self, me, him.dwId},
				{"再等等看"}
			};
...
function tbIntro:OnDialog(nIdx)
	if nIdx then
		local tbOpt = {{"我明白了。", self.OnDialog, self}};	
