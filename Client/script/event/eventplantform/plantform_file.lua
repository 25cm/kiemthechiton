	...
	for nType, szClassName in pairs(EPlatForm.MACTH_TYPE) do
		...
		if tbFileName[nType] then
			...
			if not tbTypeFile then
				print("[Nơi hoạt động] Đọc thất bại, dữ liệu không tồn tại", tbFileName[nType]);
				return
			end
			...
			if not EPlatForm.MacthType[szClassName].szName then
				EPlatForm.MacthType[szClassName].szName = "[Chưa điền hình thức]";
			end
			if not EPlatForm.MacthType[szClassName].szDesc then
				EPlatForm.MacthType[szClassName].szDesc = "[Chưa điền miêu tả]";
			end
		...
		if tbFileTrap[nType] then
			...
			if not tbTypeFile then
				print("[Nơi hoạt động] Đọc thất bại, dữ liệu không tồn tại", tbFileTrap[nType]);
				return
			end
			for nId, tbParam in ipairs(tbTypeFile) do
