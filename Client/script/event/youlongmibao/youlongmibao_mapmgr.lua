...
tbYouManager.MAX_MAP_PLAYERCOUNT	= 50;
...
function tbYouManager:CheckMapState(nMapId)
	if (not nMapId or nMapId <= 0 or SubWorldID2Idx(nMapId) < 0) then
		return 0, "Bản đồ máy chủ không tồn tại";
	end
	
	local tbMapInfo = self.tbRoomMgr.tbMapMgr[nMapId];
	if (not tbMapInfo) then
		return 0, "Thông tin bản đồ của máy chủ này không tồn tại";
	end
	
	if (not tbMapInfo.nPlayerCount or tbMapInfo.nPlayerCount >= self.MAX_MAP_PLAYERCOUNT) then
		return 0, "Số lượng người trên bản đồ rồng ở máy chủ này đã đầy, vui lòng đến các thành phố khác hoặc thôn khác";
	end
	return 1;
end
...
function tbYouManager:JoinPlayer(pPlayer)
	...
	if nMapId <= 0 then
		local pOldMe = me;
		me = pPlayer;
		Dialog:Say("Mật thất du long ở đây đã đầy, vui lòng quay thành khác và thử lại");
		me = pOldMe;
		return 0;
	end
	local nFlag, szMsg = self:CheckMapState(nMapId);
	...
	if (nRoomId <= 0) then
		local pOldMe = me;
		me = pPlayer;
		Dialog:Say("Không có phòng thích hợp để bạn vào");
		me = pOldMe;
		return 0;
	end
...
function tbYouManager:AddDialogNpc(pPlayer)
	...
	local szMsg = "<color=green>Thời gian rời khỏi còn: <color=white>%s<color>";
	self:OpenSingleUi(pPlayer, szMsg, self.TIMER_WAITPVERTIME * Env.GAME_FPS);
	self:UpdateMsgUi(pPlayer, "Đợi....");	
...
function tbYouManager:AddFightNpc(pPlayer)
	...
	local szMsg = "<color=green>Thời gian khiêu chiến còn: <color=white>%s<color>";
	self:OpenSingleUi(pPlayer, szMsg, self.TIMER_NPCPVERTIME * Env.GAME_FPS);
	self:UpdateMsgUi(pPlayer, "Đang tỷ thí.....");
...
function tbYouManager:OnDeath_FightNpc(pNpc)
	...
	if (pKillerPlayer) then	
		...
		Dialog:SendBlackBoardMsg(pPlayer, "Tỷ thí dành chiến thắng, có thể nhận thưởng");
		-- 调用秘宝开始接口
...
function tbYouManager:OnTimer_WaitTimerOver(nMapId, nRoomId)
	...
	if (tbRoomInfo) then
		...
		Dialog:SendBlackBoardMsg(pPlayer, "Bạn đã ở quá lâu trong mật thất, hãy ra ngoài hít thở");
		if (tbRoomInfo.nNpcTimerOverRegId and tbRoomInfo.nNpcTimerOverRegId > 0) then
			Timer:Close(tbRoomInfo.nNpcTimerOverRegId);
			tbRoomInfo.nNpcTimerOverRegId = nil;
		end
...
function tbYouManager:OnTimer_NpcTimerOver(nMapId, nRoomId)
	...
	if (tbRoomInfo) then
		...
		--pPlayer.SetFightState(0);
		self:AddDialogNpc(pPlayer);
		Dialog:SendBlackBoardMsg(pPlayer, "Khiêu chiến không thành công, hãy thử lại");
		if (tbRoomInfo.nNpcTimerOverRegId and tbRoomInfo.nNpcTimerOverRegId > 0) then
			Timer:Close(tbRoomInfo.nNpcTimerOverRegId);
			tbRoomInfo.nNpcTimerOverRegId = nil;
		end
