...
function CoinExchange:Exchange(pPlayer, Susscess)
	...
	if nErr == 1 then
		if not Susscess then
			GCExcute({"CoinExchange:CheckExchangePayerMax_GC", pPlayer.nId});
...
function tbCoinExchange:OnDialog()
	...
	local szMsg = "礼官：符合一定条件的玩家可以将<color=red>" .. CoinExchange.ExchangeAmount ..
		"两<color>绑银兑换成<color=red>" .. CoinExchange.ExchangeAmount * CoinExchange.ExchangeRate ..
		"两<color>银两。您的江湖威望需要达到<color=green>" .. nPrestige ..
		"点<color>方能获得今天的优惠。福利每周只可领一次，不会累计到下一周，并且数量有限，先到先得，请尽快兑换。"
	local tbOpt = {
