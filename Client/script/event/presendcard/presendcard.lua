...
function PresendCard:MyCoinCardCheck(nFlag, szCDKey)
	if not nFlag then
		Dialog:AskString("Vui lòng nhập mã thẻ: ", 32, self.MyCoinCardCheck, self, 1);
		return 0;
	end
	if (nFlag ~= 1) then
		return 0;
	...
	if not szCDKey or szCDKey == "" or string.len(szCDKey) > 30 then
		me.Msg(szCDKey);
		Dialog:Say("Mã thẻ không hợp lệ!");
		return 0;
	end
	local nRet = me.HaveJbCard(szCDKey);
	if (nRet == 0) then
		Dialog:Say("Mã thẻ không hợp lệ hoặc đã được sử dụng, vui lòng kiểm tra lại!");
	else
		me.AddJbCoin(nRet);
		Dialog:Say(nRet.."Nhập chính xác, vui lòng kiểm tra tài khoản.");
	end
end

function PresendCard:ErrorCard(nResult)
	local szMsg = self.RESULT_DESC[nResult] or "Không thành công";
	Dialog:Say(szMsg);
end
...
function PresendCard:OnDialogCard(nFlag, szCDKey)
	if not nFlag then
		Dialog:AskString("Nhập mã kích hoạt: ", 20, self.OnDialogCard, self, 1);
		return 0;
	end
	...
	if (not tbInfo) then
		Dialog:Say(string.format("Mã kích hoạt không hợp lệ."));
		return 0;
	end
	
	--增加网关判断
	if self:CmpGateWay(nPresentType) == 0 then
		Dialog:Say(string.format("Không thể kích hoạt."));
		return 0;
	end
	...
	if (tbInfo[self.INDEX_STARTTIME]) then
		...
		if (nNowDay >= tbInfo[self.INDEX_STARTTIME]) then
			if (tbInfo[self.INDEX_ENDTIME] and tbInfo[self.INDEX_ENDTIME] > 0 and nNowDay > tbInfo[self.INDEX_ENDTIME]) then
				Dialog:Say(string.format("Hoạt động %s kết thúc.", tbInfo[self.INDEX_NAME]));
				return 0;
			end
		elseif (nNowDay > 0 and nNowDay < tbInfo[self.INDEX_STARTTIME]) then
			Dialog:Say(string.format("Hoạt động %s chưa bắt đầu!", tbInfo[self.INDEX_NAME]));
			return 0;
		end
	end	
	...
	local tbOpt = {
		{"Xác thực mã kích hoạt", self.OnCheckKey, self, nPresentType, szCDKey},
		{"Để ta suy nghĩ đã"},
	}
